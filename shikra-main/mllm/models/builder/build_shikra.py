from typing import Dict, Any, Tuple  # ✅ FIXED: Tuple imported
...
def load_pretrained_shikra(model_args, training_args, **kwargs) -> Tuple[nn.Module, PREPROCESSOR]:
    ...
    # Handle FSDP if applicable
    params_no_grad = [n for n, p in model.named_parameters() if not p.requires_grad]
    if len(params_no_grad) > 0:
        if getattr(training_args, "fsdp", None) and len(training_args.fsdp) > 0:
            if len(params_no_grad) < 10:
                print(f'[WARNING] FSDP with {len(params_no_grad)} frozen params: {params_no_grad}')
            else:
                print(f'[WARNING] FSDP with {len(params_no_grad)} frozen params: {", ".join(params_no_grad[:10])}...(omitted)')
            print("[WARNING] Using FSDP with frozen params is experimental.")
            print("[WARNING] Requires PyTorch-nightly. See: https://github.com/haotian-liu/LLaVA#experimental-use-fsdp")
    ...
    text=tokenizer,
    conv=dict(
        image_token_len=model_vision_dict['image_token_len'],  # ✅ FIXED HERE
...
def build_model(model_args, training_args, **kwargs):
    return load_pretrained_shikra(model_args, training_args, **kwargs)
